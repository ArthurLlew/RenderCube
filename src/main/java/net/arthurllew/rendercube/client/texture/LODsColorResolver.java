...
public class LODsColorResolver implements AutoCloseable {
    ...
    protected List<QuadSprite> getSpritesForDirection(Level level, BlockState state, BlockPos pos, Direction facing) {
        ...
        // Get block model extra data
        ModelData blockModelData;
        ModelDataManager dataManager = level.getModelDataManager();
        if (dataManager == null) {
            blockModelData = ModelData.EMPTY;
        }
        else {
            blockModelData = dataManager.getAt(pos);
        }
        blockModelData = blockModel.getModelData(level, pos, state, blockModelData);

        // Compute list of quad sprites
        List<QuadSprite> quadSprites = new ArrayList<>();
        // Iterate render types
        for (RenderType type : RenderType.chunkBufferLayers()) {
            // Model is valid
            if (RenderShape.MODEL.equals(state.getRenderShape())) {
                // Try to collect quads facing requested direction
                List<BakedQuad> quads = blockModel.getQuads(state, facing, RandomSource.create(), blockModelData, type);
                // If no quads face that direction collect all quads instead
                if (quads.isEmpty()) {
                    quads = blockModel.getQuads(state, null, RandomSource.create(), blockModelData, null);
                }
                // Is still not empty
                if (!quads.isEmpty()) {
                    // Iterate quads
        ...
        if (!fluid.isEmpty()) {
            // Fluid tint
            int fluidTint = IClientFluidTypeExtensions.of(fluid).getTintColor(fluid, level, pos);
            // Iterate fluid sprites
            for (TextureAtlasSprite fluidSprite : FluidSpriteCache.getFluidSprites(level, pos, fluid)) {
